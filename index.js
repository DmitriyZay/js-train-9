...
function findElementGreaterThan(arr, num) {
  ...
  if (Array.isArray(arr) === true) {
    const firstBigger = arr.find((number) => number > num);
    if (firstBigger !== undefined) {
      return firstBigger;
    } else {
...
function findIndexByProperty(arr, name) {
  ...
  if (Array.isArray(arr) === true) {
    const index = ({ name: newname }) => newname === name;
    // const firstBigger = arr.find((number) => number > num);
    //const userBigAge=userList.find(({age})=> age>=minAge);
    // const userBigAge=userList.find    (({age})=> age>=minAge); //findLast,  findLastIndex
...
function findLastEvenNumber(arr) {
  ...
  if (Array.isArray(arr)) {
    const lastIndex = (index) => index % 2 === 0;
    if (arr.findLast(lastIndex)) {
      return arr.findLast(lastIndex);
    } else {
...
function getPositiveArrayValues(arr) {
  ...
  if (Array.isArray(arr) === true) {
    ...
    const newarr = Array.from(iterator);
    const result = newarr.filter((num) => num > 0);
    return result;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function sumAndPrint(arr) {
  ...
  if (Array.isArray(arr) === true) {
    const sum = arr.reduce((sum, currentnum) => sum + currentnum, 0);
...
/**
 * Функція `flattenSquareAndFilter` вирівнює масив, підносить кожен елемент до квадрату і
 *  фільтрує по заданому порогу.
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  if (Array.isArray(arr) === true) {
    arr = arr.flat();
    arr = arr.map((x) => x ** 2);
    arr = arr.filter((num) => num > threshold);
    return arr;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (Array.isArray(arr) === true) {
    const arr1 = arr.slice(n * -1);
    const arrReduce = arr1.reduceRight(
      (acum, currentnum) => acum + currentnum,
      0
    );
...
/**
 * Функція `wordLengths` приймає масив слів та повертає новий масив,
 * де кожний елемент - це довжина відповідного слова.
 *  arr - вхідний масив слів.
 * Повертає - lengthsArray - новий масив,
 * де кожний елемент - це довжина відповідного слова з вхідного масиву.
 */
...
function expandArrayByN(arr, n) {
  ...
  // Використовуємо метод flatMap для створення нового масиву,
  //де кожне число повторюється n разів
  // Повертаємо розширений масив
  if (Array.isArray(arr) === true && typeof n === "number") {
...
function findLongestWord(arr) {
  ...
  if (Array.isArray(arr) === true) {
	const longerstWord=arr.reduce((longestword, curentword)=> 
	longestword.length>curentword.length?longestword=longestword:longestword=curentword);
	return longerstWord;
  } else {
    return "           ";
  }
...
Функція findDuplicateElements приймає масив чисел і повертає новий масив, 
в якому лише дубльовані елементи.
...
function findDuplicateElements(arr) {
  ...
  if (Array.isArray(arr) === true) {
	const arr1=arr.filter((element,index)=>arr.indexOf(element)===index?false:true);
	return arr1;
  } else {
    return [];
  }
...
Функція capitalizeWords приймає масив рядків і повертає новий масив, 
де кожне слово має першу літеру у верхньому регістрі.
...
function capitalizeWords(arr) {
  ...
  // Перетворюємо першу літеру слова у верхній регістр 
  //да додамо до неї всі символи слова крім першого
  //Повертаємо слово
...
if (Array.isArray(arr)===true){
	const arr1=arr.reduce((sum,{price:num})=>sum+num,0);
	return arr1;
...
function findLastNegativeNumberIndex(arr) {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
  // Використовуємо метод findLastIndex() для знаходження 
  //індексу останнього входження від'ємного числа
